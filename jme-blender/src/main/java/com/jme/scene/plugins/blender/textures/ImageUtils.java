...
public final class ImageUtils {
    ...
    public static Image createEmptyImage(Format format, int width, int height, int depth) {
        ...
        if (depth < 2) {
            return new Image(format, width, height, BufferUtils.createByteBuffer(bufferSize), com.jme.texture.image.ColorSpace.Linear);
        }
        ArrayList<ByteBuffer> data = new ArrayList<ByteBuffer>(depth);
        for (int i = 0; i < depth; ++i) {
            data.add(BufferUtils.createByteBuffer(bufferSize));
        }
        return new Image(Format.RGB8, width, height, depth, data, com.jme.texture.image.ColorSpace.Linear);
    }
    ...
    public static Image decompress(Image image) {
        ...
        Image result = depth > 1 ? new Image(Format.RGBA8, image.getWidth(), image.getHeight(), depth, dataArray, com.jme.texture.image.ColorSpace.Linear) :
                                   new Image(Format.RGBA8, image.getWidth(), image.getHeight(), dataArray.get(0), com.jme.texture.image.ColorSpace.Linear);
    ...
    private static Image toJmeImage(BufferedImage bufferedImage, Format format) {
        ...
        return new Image(format, bufferedImage.getWidth(), bufferedImage.getHeight(), byteBuffer, com.jme.texture.image.ColorSpace.Linear);
    }
}
